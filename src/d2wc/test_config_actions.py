"""Generic add/delete actions for the dedicated d2wc UI test config."""
...
from d2wc.core.route_operations import RouteOperationError, add_route_rule_to_source, delete_route_rule_from_source
...
MANAGED_ACTION_OPERATIONS = ("add", "delete")


@dataclass(frozen=True)
class ManagedSectionActionRequest:
    """One add/delete request from the GTK managed-section form."""
...
def _apply_to_source(source: str, request: ManagedSectionActionRequest):
    ...
    if section == "EXCLUDE":
        return _add_or_delete_rule(source, operation, request.rule, add_exclude_rule_to_source, delete_exclude_rule_from_source)
    if section == "PIN":
        return _add_or_delete_rule(source, operation, request.rule, add_pin_rule_to_source, delete_pin_rule_from_source)
    if section == "WORKSPACE_ROUTES":
        if operation == "add":
            if request.workspace is None:
                raise ValueError("WORKSPACE_ROUTES add requires a workspace number")
            return add_route_rule_to_source(source, request.workspace, request.rule)
        return delete_route_rule_from_source(source, request.rule)
    if section == "GEOM":
        if operation == "add":
    ...
    if section == "WORKSPACE_PLACEMENT":
        return _add_or_delete_rule(source, operation, request.rule, add_placement_rule_to_source, delete_placement_rule_from_source)
    if section == "LEFT_EDGE_CORRECTION":
        return _add_or_delete_rule(source, operation, request.rule, add_left_edge_rule_to_source, delete_left_edge_rule_from_source)

    raise ValueError(f"Unsupported section: {request.section}")


def _add_or_delete_rule(source: str, operation: str, rule: str, add_callback, delete_callback):
    if not rule.strip():
        raise ValueError("Rule is required")
    if operation == "add":
        return add_callback(source, rule)
    return delete_callback(source, rule)


def _geometry_profile_from_request(request: ManagedSectionActionRequest) -> GeometryProfile:
    if not request.profile_name.strip():
        raise ValueError("GEOM add requires a profile name")
    missing = [name for name, value in (("x", request.x), ("y", request.y), ("w", request.w), ("h", request.h)) if value is None]
    if missing:
        raise ValueError(f"GEOM add requires numeric values for: {', '.join(missing)}")
...
def _success_message(request: ManagedSectionActionRequest) -> str:
    ...
    if section == "WORKSPACE_ROUTES" and operation == "add":
        return f"Added WORKSPACE_ROUTES rule on workspace {request.workspace}: {request.rule}"
    return f"{operation.title()}ed {section} rule: {request.rule}"
