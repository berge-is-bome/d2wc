...
The configurator manages user-facing rules and writes changes back to the managed Lua file. This document describes the configurator behavior for the rule sections that users edit through the UI.
...
For historical context behind these configurator sections and the current grammar, see [Lua Design History Notes](lua-design-history.md).
