...
The `1080` template is still only a starting point. Different desktops, themes, window borders, font sizes, and display layouts can make the same values feel slightly different from one system to another. The goal is not to make every window perfect immediately. The goal is to make the first install usable, then let the you fine-tune from there.
...
![Window geometry workflow](docs/user/images/d2wc-configurator-workspace-placement.png)
